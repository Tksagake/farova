  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const { data: loanData, error: loanError } = await supabase
        .from("loans")
        .select("amount_paid, total_due, balance_due, status")
        .eq("id", selectedLoanId)
        .single();

      if (loanError || !loanData) throw loanError || new Error("Loan not found");

      const currentAmountPaid = loanData.amount_paid || 0;
      const currentBalanceDue = loanData.balance_due || loanData.total_due;
          ...
          <div className="space-y-6">
              ...
              >
                {loans.map((loan) => (
                  <option key={loan.id} value={loan.id}>
                    {loan.purpose} (KES {loan.amount_requested.toLocaleString()}) - {loan.status}
                  </option>
                ))}
              </select>
            ) : (
            ...
            {selectedLoan && (
              <p className="text-lg font-semibold">
                Balance Due: KES {selectedLoan.balance_due.toLocaleString()}
              </p>
            )}
            ...
            {/* File Upload */}
            <input type="file" accept=".pdf,.jpg,.jpeg,.png" className={`w-full bg-blue-600 text-white py-3 px-4 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-300 ease-in-out ${loading ? "opacity-70" : ""}`} onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
